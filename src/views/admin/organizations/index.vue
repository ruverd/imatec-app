<template>
  <div class="mixin-components-container">
    <el-row style="margin-bottom:32px;">
      <el-card class="box-card">
        ...
        <div class="clearfix">
          ...
          <el-button-group style="float: right;">
            <el-tooltip class="item" effect="dark" content="Adicionar" placement="top-end">
              <el-button v-waves plain icon="el-icon-plus" size="medium" @click="openDialog()"></el-button>
            </el-tooltip>
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
          ...
          <el-form-item>
            <el-button v-waves type="primary" @click="searchForm('ruleForm')" icon="el-icon-search">Pesquisar</el-button>
    ...
    <!-- Dialog -->
    <el-dialog title="Adicionar Organização" :visible.sync="dialog" width="90%" top="10">
...
export default {
  ...
  computed: {
    dialog() {
      return this.$store.state.organization.dialog
    }
  },
  methods: {
    openDialog() {
      console.log('loading')
      this.$store.dispatch('DialogVisibled', true).then(() => {
        console.log('finish')
      }).catch(() => {
        console.log('erro')
      })
    ...
    searchForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$notify({
            title: 'Erro',
            message: `Sucesso ao realizar busca!`,
            type: 'warning'
          })
        } else {
          this.$notify({
    ...
    loadData(queryInfo) {
      ...
      return request.get('/organizations', { params: queryInfo })
        .then(response => {
